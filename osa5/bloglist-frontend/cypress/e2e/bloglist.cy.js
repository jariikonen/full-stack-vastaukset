  ...
  describe('When logged in', function() {
    ...
    describe('When blogs have been created', function() {
      beforeEach(function() {
        ...
        cy.contains(`${testBlogs[0].title} ${testBlogs[0].author}`).find('button').contains('view').click();
      });
      ...
      it('A blog can be removed by the user who created it', function() {
        ...
        cy.get('html').should('not.contain', `${testBlogs[0].title} ${testBlogs[0].author}`);
      });

      it('Remove button is rendered only if logged in user has created the blog', function() {
        cy.contains(`${testBlogs[0].title} ${testBlogs[0].author}`).parent().as('theBlog');
      ...
      it('Blogs are rendered in descending order by the number of likes they have', function() {
        ...
        cy.get('.blog').eq(0).contains(`${testBlogs[2].title} ${testBlogs[2].author}`);
        cy.get('.blog').eq(1).contains(`${testBlogs[1].title} ${testBlogs[1].author}`);
        cy.get('.blog').eq(2).contains(`${testBlogs[0].title} ${testBlogs[0].author}`);
      });
    });
  });
});
